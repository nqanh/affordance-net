...
class coco(imdb):
    ...
    def _load_image_set_index(self):
        ...
        Load image ids.
        """
        image_ids = self._COCO.getImgIds() #lay tat ca image indexes
        return image_ids
    ...
    def _roidb_from_proposals(self, method):
        ...
        if osp.exists(cache_file):
            ...
            print '============_roidb_from_proposals:======{:s} {:s} roidb loaded from {:s}'.format(self.name, method,
                                                            cache_file)
            return roidb
        ...
        print '=========My gt 2=========wrote {:s} roidb to {:s}'.format(method, cache_file)
        return roidb
    ...
    def gt_roidb(self):
        ...
        if osp.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                roidb = cPickle.load(fid)
            print '=========gt_roidb:====={} gt roidb loaded from {}'.format(self.name, cache_file)
            return roidb

        #load annotation cua TAT CA cac image trong database
        gt_roidb = [self._load_coco_annotation(index) for index in self._image_index] #self._image_index: index cua tat ca cac images trong database

        with open(cache_file, 'wb') as fid:
            cPickle.dump(gt_roidb, fid, cPickle.HIGHEST_PROTOCOL)
        print '=====gt_roidb:=====wrote gt roidb to {}'.format(cache_file)
        return gt_roidb
    ...
    def _load_coco_annotation(self, index):
        ...
        for obj in objs:
            ...
            x2 = np.min((width - 1, x1 + np.max((0, obj['bbox'][2] - 1))))  #col
            y2 = np.min((height - 1, y1 + np.max((0, obj['bbox'][3] - 1)))) #row
            if obj['area'] > 0 and x2 >= x1 and y2 >= y1:
                obj['clean_bbox'] = [x1, y1, x2, y2]
                ################# get segmentation mask of this object###############################################
                if cfg.TRAIN.MASK_REG:
                    count = count + 1
                    obj['seg_mask_ind'] = [index, count] #image index and order of this object in the image
        ...
        for ix, obj in enumerate(objs): #moi element cua objs la 1 obj voi nhieu fields nhu 'category_id','clean_bbox',.... Chu ban than objs ko co cac fields nhu tren. objs chi chua obj
    ...
    def _do_detection_eval(self, res_file, output_dir):
        if cfg.TEST.TEST_INSTANCE:
            ann_type = 'segm'
            eval_file = osp.join(output_dir, 'instance_results.pkl')
        else:
            ann_type = 'bbox'
    ...
    def _coco_results_one_category(self, boxes, rles, cat_id):
        ...
        for im_ind, index in enumerate(self.image_index):
            if cfg.TEST.TEST_INSTANCE:
                dets = rles[im_ind] #rles[im_ind]: one class, one image
                dets_box = boxes[im_ind].astype(np.float)
                ...
                if len(dets) == 0:#if dets == {}:
                    continue

                scores = dets_box[:, -1] #class score
                segmentations = dets[:]
                ...
                results.extend(
                    [{'image_id': index,
                      'category_id': cat_id,
                      'segmentation': segmentations[k],
                      'score': scores[k]} for k in xrange(dets.shape[0])])
                # print "============results============"
                # print results
            else:
                dets = boxes[im_ind].astype(np.float)#one class, one image
                if dets == []:
                    continue
                scores = dets[:, -1]
                xs = dets[:, 0]
    ...
    def _write_coco_results_file(self, all_boxes, all_rles, res_file):
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            # if cfg.TEST.TEST_INSTANCE:
            #     results.extend(self._coco_results_one_category(all_boxes[cls_ind], all_rles[cls_ind], coco_cat_id)) #all_rles[cls_ind]: one class, all images
            # else:
            #     results.extend(self._coco_results_one_category(all_boxes[cls_ind], all_rles[cls_ind], coco_cat_id))#all_boxes[cls_ind]: one class, all images
            results.extend(self._coco_results_one_category(all_boxes[cls_ind], all_rles[cls_ind], coco_cat_id))  # all_rles[cls_ind]: one class, all images
    ...
    def evaluate_detections(self, all_boxes, all_rles, output_dir):
        ...
        if cfg.TEST.TEST_INSTANCE:
            res_file = osp.join(output_dir, ('instances_' +
                                         self._image_set +
                                         self._year +
                                         '_results'))
        else:
        ...
        res_file += '.json'
        self._write_coco_results_file(all_boxes, all_rles, res_file) #all_boxes/all_rles: all classes, all images
