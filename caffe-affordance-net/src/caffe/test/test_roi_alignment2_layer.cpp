...
class ROIAlignment2LayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ROIAlignment2LayerTest()
      : blob_bottom_data_(new Blob<Dtype>(4, 3, 12, 8)),//blob_bottom_data_(new Blob<Dtype>(4, 3, 12, 8)),  //N: batch = 4, #Channel = 3. height = 12, width = 8. spatial scale = 1
        blob_bottom_rois_(new Blob<Dtype>(4, 5, 1, 1)),   //num_rois: 1 spatial_scale: 1.000000 channels: 1
        blob_top_data_(new Blob<Dtype>())
 {
    // fill the values
    FillerParameter filler_param;
    filler_param.set_std(10.0);
  ...
  virtual ~ROIAlignment2LayerTest() {
...
TYPED_TEST_CASE(ROIAlignment2LayerTest, TestDtypesGPU);

TYPED_TEST(ROIAlignment2LayerTest, TestGradient)
{
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ROIAlignment2Parameter* roi_alignment2_param = layer_param.mutable_roi_alignment2_param();
  roi_alignment2_param->set_pooled_h(6);
  roi_alignment2_param->set_pooled_w(6);
  ROIAlignment2Layer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-3, 1e-2);//checker(1e-4, 1e-2); //the first number is for float test (0.TestGradient <->caffe::GPUDevice<float>)
